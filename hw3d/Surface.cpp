...
Surface::Surface( unsigned int width, unsigned int height ) noexcept	//Blank surface constructor
	:
...
Surface& Surface::operator=( Surface&& donor ) noexcept	//Passes in rvalue Donor to set width, height & buffer properties
{
	width = donor.width;
	height = donor.height;
...
Surface::Surface( Surface&& source ) noexcept	//Initialised buffer constructor
	:
...
void Surface::Clear( Colour fillValue ) noexcept
{
	memset( pBuffer.get(), fillValue.dword, width * height * sizeof( Colour ) );
}

void Surface::PutPixel( unsigned int x, unsigned int y, Colour c ) noexcept(!IS_DEBUG)
{
	assert( x >= 0 );
	assert( y >= 0 );
	assert( x < width );
...
Surface::Colour* Surface::GetBufferPtr() noexcept
{
	return pBuffer.get();
}

const Surface::Colour* Surface::GetBufferPtr() const noexcept
{
	return pBuffer.get();
}
